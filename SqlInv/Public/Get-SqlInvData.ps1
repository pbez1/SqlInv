...
function Get-SqlInvData {
    ...
    if ($Instance_list) {
        Clear-PsTableContents -TableName 'db_staging'
        }
    ...
    foreach ($inst in $instance_list) {
        try {
            ...
            New-LogEntry @LoggingParms -LogMessage $msg
            ...
            # Get some of the parameters the SQL statement will be unable to obtain.
            $agent_email_eanbled = Get-AgentEmailEnabled -ServerInstance $instance_nm
            $searched_domain = Get-SearchedDomain -Server $server_nm
            $svc_account = Get-ServiceLogonAccount -Server $server_nm -ServiceDesc 'SQLServer' -InstanceName $named_instance
            $agt_account = Get-ServiceLogonAccount -Server $server_nm -ServiceDesc 'SQLAgent' -InstanceName $named_instance
            $is_vm = Get-IsVM -Server $instance_nm
